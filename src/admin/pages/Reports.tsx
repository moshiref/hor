...
export default function Reports() {
  ...
  return (
    <div className="space-y-6">
      <h1 className="font-display text-xl font-bold text-ink-800">التقارير</h1>

      <div className="rounded-2xl bg-white p-4 shadow-card space-y-4">
        <div className="grid gap-3 sm:grid-cols-5">
          <select value={type} onChange={(e) => setType(e.target.value as ReportType)} className="rounded-xl border border-ink-100 px-3 py-2 text-sm">
            <option value="students">طلاب</option>
            <option value="staff">عاملات</option>
          </select>
          <select value={status} onChange={(e) => setStatus(e.target.value)} className="rounded-xl border border-ink-100 px-3 py-2 text-sm">
          ...
          <select value={sort} onChange={(e) => setSort(e.target.value as 'newest' | 'oldest')} className="rounded-xl border border-ink-100 px-3 py-2 text-sm">
        ...
        <div className="flex gap-2">
          ...
          <button onClick={() => printTable('report-table')} className="rounded-full border border-ink-200 px-4 py-1.5 text-sm font-bold text-ink-700">طباعة</button>
        </div>

        <div id="report-table" className="rounded-xl border border-ink-100 p-4">
          <h2 className="text-center font-display text-lg font-bold text-ink-800">{title}</h2>
          <p className="mt-1 text-center text-xs text-ink-400">تاريخ التقرير: {new Date().toLocaleString('ar-SA')} — {data.length} نتيجة</p>
                ...
                {data.length === 0 ? <tr><td colSpan={5} className="p-6 text-center text-ink-400">لا توجد بيانات</td></tr> : data.slice(0, 100).map((r: unknown) => {
                  ...
                  return (
                    <tr key={(s as { id: string }).id} className="border-b border-ink-100">
                      <td className="p-2 text-center">{(s as StudentApplication).studentName ?? (s as StaffApplication).fullName}</td>
                      <td className="p-2 text-center">{(s as StudentApplication).guardianName ?? (s as StaffApplication).currentEmployer}</td>
                      <td className="p-2 text-center"><bdi dir="ltr">{(s as StudentApplication).guardianPhone ?? (s as StaffApplication).phone}</bdi></td>
                      <td className="p-2 text-center">{(s as { status: string }).status}</td>
                      <td className="p-2 text-center text-xs">{new Date((s as { createdAt: string }).createdAt).toLocaleDateString('ar-SA')}</td>
                    </tr>
                  )
                })}
              </tbody>
